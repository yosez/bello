...
ExpStrc* bldAsgnExp(ExpStrc* lvl, ExpStrc* exp);
ExpStrc* bldVrbExp(char* idtf);
ExpStrc* bldBnrExp(int opr, ExpStrc* lftExp, ExpStrc* rghtExp);
ExpStrc* blnTrpExp(int opr, ExpStrc* frstExp, ExpStrc* scndExp, ExpStrc* trdExp);
ExpStrc* bldUnrExp(int opr, ExpStrc* exp);
...
ExpStrc* bldTnrExp(int opr, ExpStrc* frstExp, ExpStrc* scndExp, ExpStrc* trdExp)
{
...
ValStrc* clcBnrExpAdd(vector<EnvrStrc*>& envr, struct BnrExpStrc* exp)
{
	ValStrc* lftCnst = clcExp(envr, exp->lftExp);
	ValStrc* rghtCnst = clcExp(envr, exp->rghtExp);
	...
	{
		str = *(lftCnst->v.str) + *(rghtCnst->v.str);

		rslt = bldStrValByStr(str);
	}

	if (lftCnst->typ == ValEnm::Str && rghtCnst->typ == ValEnm::Int)
	{

		str = *(lftCnst->v.str);

		str += to_string(rghtCnst->v.int_);

		rslt = bldStrValByStr(str);
	}

	if (lftCnst->typ == ValEnm::Int && rghtCnst->typ == ValEnm::Str)
	{

		str = to_string(lftCnst->v.int_);
		str += *(rghtCnst->v.str);

		rslt = bldStrValByStr(str);
	}

	if (lftCnst->typ == ValEnm::Str && rghtCnst->typ == ValEnm::Flt)
	{

		str = *lftCnst->v.str;
		str += to_string(rghtCnst->v.flt);

		rslt = bldStrValByStr(str);
	}

	if (lftCnst->typ == ValEnm::Flt && rghtCnst->typ == ValEnm::Str)
	{

		str = to_string(lftCnst->v.flt);
		str += *(rghtCnst->v.str);

		rslt = bldStrValByStr(str);
	}

	if (lftCnst->typ == ValEnm::Str && rghtCnst->typ == ValEnm::Bln)
	{

		str = *(lftCnst->v.str);

		str += rghtCnst->v.bln == 0 ? "false" : "true";

		rslt = bldStrValByStr(str);
	}

	if (lftCnst->typ == ValEnm::Bln && rghtCnst->typ == ValEnm::Str)
	{

		str = lftCnst->v.bln == 0 ? "false" : "true";
...
ValStrc* clcTnrExpQm(vector<EnvrStrc*>& envr, TnrExpStrc* exp)
{
	...
	if (clcExp(envr, exp->frstExp)->v.int_ != 0)
	{
